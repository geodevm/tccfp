...
collars.all <- spTransform(collars.all, CRS("+proj=utm +zone=15 +ellps=GRS80 +datum=NAD83 +no_defs"))
...
# polygon, which really tells you nothing about use within that polygon.
...
# From there you can just do what we did with 95% KDEs. For example:


F1.kde <- kernelUD(F1.sp, h = "href")
image(F1.kde)
F1.rast <- raster(as(F1.kde, "SpatialPixelsDataFrame"))
plot(F1.rast)
plot(F1.sp, add = T)
writeRaster(F1.rast, "F1.kde.tif")
F1.kde95 <- getverticeshr(F1.kde, percent = 95, unin = "m", unout = "km2")
plot(F1.rast)
plot(F1.sp, add = T)
plot(F1.kde95, add = T)

# To export as shapefiles - Make sure to set the working directory beforehand.
setwd()
