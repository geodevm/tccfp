...
library(tidyverse)
# Read in GPS data -------------------------------------------------------------
gps <- tibble(read.csv(here("data/processed_data/hr_behavior_appended.csv")))
...
for (i in 1:3) {
  tbl_name <- trks_names[i]
  firsts[[i]] <- firsts[[i]] %>% dplyr::select(!7:9)
...
rm(burst, dir, drops, firsts_i, gps_11_h, gps_5_h, gps_10_m, i, j, newnames, 
   oldnames, sl, ta, tbl, tbl_name, trks_names)
...
drops  <- c("x2_", "y2_", "t2_", "dt_")
first_steps <- first_steps %>% dplyr::select(!5:7)
...
rm(drops, first_steps, gps_steps, gps_steps_i, gps_trk, i, lvls, newnames, 
   oldnames)
