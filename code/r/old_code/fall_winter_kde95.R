...
summary(collars)
# Fall/winter
collars_kde <- collars[((collars$acquisition_time >= ymd_hms("2019-09-01 00:00:00") & 
                       collars$acquisition_time <= ymd_hms("2020-03-15 00:00:00")) |
                    (collars$acquisition_time >= ymd_hms("2020-09-01 00:00:00") & 
                       collars$acquisition_time <= ymd_hms("2021-03-15 00:00:00")) |
                    (collars$acquisition_time >= ymd_hms("2021-09-01 00:00:00") & 
                       collars$acquisition_time <= ymd_hms("2022-03-15 00:00:00"))),]
# Overdispersed individuals without home ranges.
collars_kde <- collars_kde[collars_kde$animal_id != "F14", ]
collars_kde <- collars_kde[collars_kde$animal_id != "F18", ]
collars_kde <- collars_kde[collars_kde$animal_id != "F5", ]
collars_kde <- collars_kde[collars_kde$animal_id != "F4", ]
collars_kde <- collars_kde[collars_kde$animal_id != "F10", ]
collars_kde <- collars_kde[!(collars_kde$animal_id == "C1" & collars_kde$acquisition_time > ymd_hms("2020-01-16 00:00:00")),]
collars_kde <- collars_kde[!(collars_kde$animal_id == "C12" & collars_kde$acquisition_time < ymd_hms("2021-09-01 00:00:00")),]
droplevels(collars_kde)

# Convert the longitude and latitude columns to spatial points (think of as x = longitude, y = latitude for plotting)
sp_collars <- SpatialPoints(collars_kde[c("gps_longitude", "gps_latitude")])

# Plot these spatial points to get a look
plot(sp_collars)

id <- as.list(levels(as.factor(as.character(collars_kde$animal_id))))
for (i in id) {
  i_collar <- collars_kde[collars_kde$animal_id == i, ]
  plot(i_collar$gps_longitude, i_collar$gps_latitude, main = i_collar$animal_id[1])
}
